      ...
      {shown.map(([emoji, n]) => {
        ...
        return (
          <button
            ...
            aria-pressed={on}
            className={`flex items-center gap-1 rounded-full border px-2 py-0.5 text-[13px] transition-[background-color,border-color,color,transform,rotate,scale,translate] duration-200 hover:-translate-y-px active:scale-95 ${
      ...
      <Popover
        button={({ toggle }) => (
          <button
            ...
            className="grid size-7 place-items-center rounded-full border border-line text-ink-3 transition-[color,border-color,transform,rotate,scale,translate] duration-200 hover:rotate-12 hover:border-primary hover:text-primary"
          >
            ...
            <div className="flex gap-1 rounded-[var(--radius-card)] border border-line bg-paper p-1.5">
              {QUICK.map((e) => (
                <button key={e} type="button" onClick={() => { onToggle(e); close(); }}
                  className="grid size-8 place-items-center rounded-[var(--radius-ui)] text-[17px] transition-[transform,rotate,scale,translate] duration-150 hover:scale-110 hover:bg-surface active:scale-95">{e}</button>
              ))}
            </div>
